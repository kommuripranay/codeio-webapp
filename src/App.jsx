...
// REMOVED Code2 from imports

// --- 1. Navbar Component (Defined OUTSIDE App) ---
const Navbar = ({ onOpen }) => (
  <nav className="fixed top-0 left-0 w-full z-50 px-6 py-5 flex justify-between items-center backdrop-blur-md bg-cyber-black/30">
    {/* Logo Area - Removed Code2 Icon */}
    <div className="flex items-center gap-2 select-none cursor-pointer">
      {/* Code2 icon was here, now removed */}
    ...
    <button 
      onClick={onOpen}
      className="p-2 rounded-full hover:bg-cyber-accent/10 transition-colors duration-300 active:scale-95 text-cyber-text"
    >
      ...
      <>
        ...
        >
          <div className="flex justify-end mb-12">
            <button 
              onClick={onClose}
              className="p-2 hover:rotate-90 transition-transform duration-500 text-cyber-text"
            >
  ...
  <section className="min-h-screen w-full flex flex-col justify-center items-center px-4 relative overflow-hidden pt-20">
    ...
    >
      ...
      <motion.button 
        ...
        whileTap={{ scale: 0.95 }}
        className="mt-12 px-8 py-3 border border-cyber-gray rounded-full text-sm hover:bg-cyber-accent hover:text-black transition-all duration-300 tracking-wide font-medium text-cyber-text"
      >
        INITIALIZE PROJECT
      </motion.button>
    </motion.div>
    ...
    <section className="py-24 px-6 md:px-12 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        {uploads.map((item, idx) => (
          ...
          >
            <div className="absolute top-0 right-0 p-4 opacity-0 group-hover:opacity-100 transition-opacity text-cyber-text">
              <ArrowRight size={20} />
            </div>
